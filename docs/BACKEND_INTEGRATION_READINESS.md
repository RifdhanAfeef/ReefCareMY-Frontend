...
| US1.6 Registration | Real public observer-only registration; no role selector; 6–128 characters and at least four distinct characters; separate protected administrator account-creation form for privileged roles | Frontend registration matches the Iteration 1 six-character minimum. The deployed backend currently reports an obsolete 12-character minimum and must be aligned. Administrator provisioning is blocked by the missing administrator API. |
...
- `temporaryPassword` — required, 6–128 characters with at least four distinct
  characters; and
...
- The Iteration 1 frontend contract requires 6–128 password characters and at
  least four distinct characters. The deployed registration schema must use the
  same six-character minimum.
